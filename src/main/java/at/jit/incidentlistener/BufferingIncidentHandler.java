...
public class BufferingIncidentHandler extends DefaultIncidentHandler {
  ...
  IncidentInformation createIncidentInfo(final IncidentEntity incEnt) {
    ...
    return new IncidentInformation().withType(incEnt.getIncidentType())
        .withActivityId(incEnt.getActivityId())
        .withMessage(clean(incEnt.getIncidentMessage()))
        .withProcessInstanceId(incEnt.getProcessInstanceId())
  ...
  boolean sendEmail(final RecipientInfo recipientInfo, final String messageText) {
    ...
    mailConfig.put("mail.smtp.ssl.enable", Boolean.toString(config.getEnableSSL()));
    mailConfig.put("mail.smtp.starttls.enable", Boolean.toString(config.getEnableTLS()));
